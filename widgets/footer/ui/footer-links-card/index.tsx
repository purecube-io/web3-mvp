...
export const FooterLinksCard = ({ title, elems }: FooterLinksCardProps) => {
  return (
    <div className="space-y-3">
      <span className="text-base md:text-lg font-heading-semibold uppercase">
        {title}
      </span>
        ...
        {elems.map((elem, index) => {
          return (
            <li key={index} className="flex items-center gap-3">
              ...
              {elem.icon === 'telegram' && (
                <FontAwesomeIcon className="h-5" icon={faTelegram} />
              )}
              {elem.soon ? (
              ...
              {elem.soon && (
                <span className="inline-block text-xs text-black bg-neutral-400 px-1 rounded-full select-none">
                  soon
                </span>
              )}
            </li>
