...
export const GameListItem = (game: GameListItemProps) => {
  return (
    <div
      className={`grid grid-cols-1 xl:grid-cols-2 gap-3 md:gap-5 items-center p-3 md:p-5 rounded-3xl ${game.bgTransparent ? 'bg-transparent' : 'bg-neutral-900'}`}
    >
      ...
      <div className="flex flex-col sm:items-start space-y-3">
        ...
        <div
          className={`grid grid-cols-1 sm:grid-cols-3 gap-3 md:gap-5 w-full ${game.showDetails ? 'grid' : 'hidden'}`}
        >
          <div className="flex flex-col">
            <span className="text-base md:text-lg font-body-semibold">
              Genre
            </span>
